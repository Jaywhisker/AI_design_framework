...
def get_youtube_captions(search_terms, api_key, count):
...
def get_title(vid_id, youtube):
    ...
    return title, video_response #we are also returning video_response (contains video information) as we will need it to get video_comments
...
def youtube_captions(vidID, youtube):
    ...
    for i in range(len(vidID)):
        try:
            ...
            captions = YouTubeTranscriptApi.get_transcript(vid_id[i])
            cleaned_captions = ""
            ...
            df = df.append({"title": title, "caption": cleaned_captions}, ignore_index = True)
